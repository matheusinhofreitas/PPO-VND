...
#  
GRUPO_INSTANCIA = 10 #10 15 30 50
TIPO_RECOMPENSA = 2 # 0 1 2 3

CARREGAR_MODELO = False
CONTINUAR_TREINAMENTO = False
#NOME_MODELO = "MODELO_36_inst_UB=FO_inicial"
#NOME_MODELO = "MODELO_36_inst_1M"
NOME_MODELO = "MODELO_passos_"+str(PASSOS_TREINAMENTO)+"_Recompensa_"+str(TIPO_RECOMPENSA)+"_inst_N_"+str(GRUPO_INSTANCIA)
...
#salva Resultados PPO
  myfile = open(str(NOME_MODELO)+"_resultados_PPO_"+str(i)+"+.txt", "w")
...
for i in range(5):
  ...
  myfile = open(str(NOME_MODELO)+"_resultados_PPO_Random_"+str(i)+".txt", "w")
